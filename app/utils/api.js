 ...
 * @returns {{ properties: PropertiesListEntry[] }}
 */
...
/**
 * Retrieve details for specific properties. The API endpoint this function
 * simulates will perform extensive computations on each property, and therefore
 * the response time will vary with the number of requested property IDs.
 *
 * @param {string[]} propertyIds
 * @returns {Promise<{ properties: PropertyDetails[] }>}
 */
async function fetchPropertyDetails(propertyIds) {
  ...
  for (const id of propertyIds) {
    await wait(randomInteger(500));
    const desiredProperty = sampleProperties.find((property) => property.id === id);

    if (desiredProperty !== undefined) {
      propertiesToReturn.push(desiredProperty);
    }
  }

  return { properties: propertiesToReturn };
}
